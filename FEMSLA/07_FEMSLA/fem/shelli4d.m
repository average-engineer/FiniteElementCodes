function [Ke, Me, Ce] = shelli4d(eX, eY, eZ, eP, D)
...
%         eZ = [z1 z2 z3 z4]
%                             
%         eP =[t ir rho a b] element property 
%                           ir:  integration rule
%                           t:   thickness
%                           rho: density
%                           a, b: exprimental factors Ce = a * Ke + b * Me
%
...
%--------- element mass matrix -------------------------------
 v12 = [eX(2) - eX(1); eY(2) - eY(1); eZ(2) - eZ(1)];
 v14 = [eX(4) - eX(1); eY(4) - eY(1); eZ(4) - eZ(1)];
 v32 = [eX(2) - eX(3); eY(2) - eY(3); eZ(2) - eZ(3)];
 v34 = [eX(4) - eX(3); eY(4) - eY(3); eZ(4) - eZ(3)];
 
 A = (norm(cross(v12, v14)) + norm(cross(v32, v34))) / 2; % two triangles with the area of half their parallelograms
 
 Mel = zeros(24, 24);
 iMass = [1 2 3 7 8 9 13 14 15 19 20 21]; % translative degrees of freedom
 
 % use lumped mass matrix (mass centered in nodal points, rotation neglected)
 Mel(iMass, iMass) = rho * A * t / 4 * eye(12);
...
%--------- Mass and Damping -----------------------------------------------
 Me = Mel;  % no need to rotate mass matrix
 if (nargout == 3)
    Ce = a * Ke + b * Me;
 end
